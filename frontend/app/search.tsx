...
export default function SearchScreen() {
  ...
  const { cardWidth, numColumns } = useMemo(() => {
    ...
    // Desktop web (>768px): Fixed card width of 193px, 5px horizontal gap
    if (Platform.OS === 'web' && screenWidth > 768) {
      const FIXED_CARD_WIDTH = 193;
      const WEB_GAP = 5;
    ...
    // Mobile: Fixed 2-column layout with 5px horizontal gap, 200px card width
    const MOBILE_GAP = 5;
      ...
      {loading ? (
        ...
        <View style={[
          ...
          <FlashList
            ...
            renderItem={({ item }) => (
              <View style={[styles.cardWrapper, { width: cardWidth + (Platform.OS === 'web' ? 7 : 5) }]}>
                <ProductCard
                  product={item}
                  cardWidth={cardWidth}
